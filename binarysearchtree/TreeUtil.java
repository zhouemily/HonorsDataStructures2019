...
public class TreeUtil
{
    ...
    /**
     * Build the tree with the given iterator to iterate through a valid description of
    ...
    /**
     * A helper method for building a Morse code decoding tree.
     * 
    ...
    String code, TreeDisplay display)
    {
        ...
        String morse = code.substring(0, 1);
        String remainingCode = code.substring(1);
        if (morse.equals("."))
        {
            TreeNode node = decodingTree.getLeft();
            if (node == null)
            {
                node =  new TreeNode(null);
                decodingTree.setLeft(node);
            }
            insertMorse(node, letter, remainingCode, display);
            return;
        }
        else if (morse.equals("-"))
        {
            TreeNode node = decodingTree.getRight();
            if (node == null)
            {
                node =  new TreeNode(null);
        ...
        throw new RuntimeException("Unknown mouse code " + morse);
    }
    ...
    public static String decodeMorse(TreeNode decodingTree, String cipherText, TreeDisplay display)
    {
        ...
        display.visit(node);
        while (code.length() > 0 && !code.substring(0, 1).equals(" "))
        {
        ...
        while (code.substring(0, 1).equals(" ")) 
        {
            code = code.substring(1);    
        }
        return node.getValue() + decodeMorse(decodingTree, code, display);
    }
